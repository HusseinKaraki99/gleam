// Gleam engine — generated bundle. Do not edit by hand.
...
var require_printer = __commonJS({
  "node_modules/.pnpm/handlebars@4.7.9/node_modules/handlebars/dist/cjs/handlebars/compiler/printer.js"(exports) {
    "use strict";
    exports.__esModule = true;
    exports.print = print;
...
var require_lib = __commonJS({
  "node_modules/.pnpm/handlebars@4.7.9/node_modules/handlebars/lib/index.js"(exports, module) {
...
function resolveConfig(config) {
  ...
  const ormPerService = /* @__PURE__ */ new Map();
...
var nextjsStack = {
  ...
  nxProjectConfig: {
    ...
    targets: {
      ...
      dev: { executor: "@nx/next:server", options: { port: 3e3 } },
...
// src/stacks/postgres-drizzle.ts
var postgresDrizzleStack = {
  id: "drizzle",
...
// src/stacks/postgres-sqlc.ts
var postgresSqlcStack = {
  id: "sqlc",
  name: "Postgres + sqlc",
...
function compose(config) {
  ...
  const ciSteps = [];
  const scripts = {};
  const seenArtifactPaths = /* @__PURE__ */ new Set();
...
function addClaudeBaseArtifacts(config, artifacts, seen) {
  ...
  if (config.aiLevel === "minimal") {
    const minimalTypes = /* @__PURE__ */ new Set(["rule"]);
  ...
  if (config.aiLevel === "standard") {
    const standardTypes = /* @__PURE__ */ new Set(["rule", "agent", "skill", "hook"]);
...
function createRenderer(loadTemplate) {
...
function registerHelpers(hbs) {
  ...
  hbs.registerHelper("camelCase", (str) => {
    const pascal = str.split(/[-_\s]+/).map((word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join("");
  ...
  hbs.registerHelper("year", () => (/* @__PURE__ */ new Date()).getFullYear());
}

// src/generator/render-project.ts
function renderProjectFiles(config, loadTemplate) {
...
// src/generator/fs-template-loader.ts
import * as fs from "node:fs";
import * as path from "node:path";
function templatesDir() {
  return process.env.GLEAM_TEMPLATES_DIR ? path.resolve(process.env.GLEAM_TEMPLATES_DIR) : path.resolve(import.meta.dirname, "../../templates");
}
...
function writeFiles(targetDir, files) {
  let filesWritten = 0;
  const directoriesCreated = /* @__PURE__ */ new Set();
  const errors = [];
...
// src/generator/post-process.ts
import { execSync } from "node:child_process";
import * as path3 from "node:path";
import * as fs3 from "node:fs";
function postProcess(targetDir, config) {
  const steps = [];
...
function hasNodePackages(targetDir) {
  return fs3.existsSync(path3.join(targetDir, "package.json"));
}

// src/config/presets/solo-saas.ts
function soloSaasPreset(overrides) {
  return {
...
// src/config/presets/team-platform.ts
function teamPlatformPreset(overrides) {
  return {
...
// src/config/presets/enterprise.ts
function enterprisePreset(overrides) {
  return {
...
// src/config/presets/fullstack-mobile.ts
function fullstackMobilePreset(overrides) {
  return {
...
function generateToDisk(config, options) {
  ...
  if (write.errors.length > 0) {
    throw new GenerationError(
      targetDir,
      write.errors.map((e) => `${e.path}: ${e.error}`)
    );
  }
  const post = options.runPostProcess ? postProcess(targetDir, resolveConfig(config)) : null;
...
function parseFlags(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (arg === "--no-post-process") flags.noPostProcess = true;
    ...
    else if (arg === "--display-name") flags.displayName = argv[++i];
    else if (arg === "--description") flags.description = argv[++i];
    else if (arg === "--org") flags.org = argv[++i];
...
function main(argv) {
  ...
  if (flags.dryRun) {
    ...
    for (const file of files.map((f) => f.path).sort()) console.log(`  ${file}`);
    return;
  }
  ...
  console.log(
    `Generated ${result.write.filesWritten} files into ${path4.resolve(targetDir)} (${result.write.directoriesCreated} directories).`
  );
