 ...
 */
export function generateProject(config: ProjectConfig): RenderedFile[] {
...
export function generateToDisk(
  ...
  options: GenerateToDiskOptions,
): GenerateToDiskResult {
...
// --- CLI runner (invoked by the /init-project skill via `pnpm generate`) ---
...
function parseFlags(argv: string[]): Flags {
  const flags: Flags = { dryRun: false, noPostProcess: false };
...
function configFromFlags(flags: Flags): ProjectConfig {
  if (flags.config) {
    return JSON.parse(flags.config) as ProjectConfig;
  }
  if (!flags.preset) {
...
function main(argv: string[]): void {
  ...
  console.log(
    `Generated ${result.write.filesWritten} files into ${path.resolve(targetDir)} ` +
